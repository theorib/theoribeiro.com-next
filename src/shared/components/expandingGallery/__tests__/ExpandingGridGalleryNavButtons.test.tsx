...
describe('ExpandingGridGallery Nav Button Interactions', () => {
  test.each(testContent)('Next Button for test item #%#', async content => {
    ...
    await checkContentOfGalleryExpanderElement(indexOfCurrentTest, testContent);
    ...
    for (let j = 0; j < numberOfItemsLeftToClick; j++) {
      ...
      if (j + 1 === numberOfItemsLeftToClick)
        expect(navButtonNext).toBeDisabled();
      if (j + 1 < numberOfItemsLeftToClick) expect(navButtonNext).toBeEnabled();
  ...
  test.each(testContent)('Prev Button for test item #%#', async content => {
    const { user, galleryList, galleryItems } =
      await renderExpandingGalleryTest();
    expect(galleryList).toBeInTheDocument();
    expect(galleryItems).toHaveLength(testContent.length);
  ...
  test.each(testContent)('Close Button for test item #%#', async content => {
    const { user, galleryList, galleryItems } =
      await renderExpandingGalleryTest();
    expect(galleryList).toBeInTheDocument();
    expect(galleryItems).toHaveLength(testContent.length);
